...
async def backfill_monthly(
    ...
    if len(windows) > MAX_MONTHS_PER_RUN:
        raise ValueError(
            f"maximum {MAX_MONTHS_PER_RUN} calendar months per request; "
            "split longer historical ranges into multiple requests"
        )

    months: list[dict] = []
    totals = {
        "months": len(windows),
        "fixture_days_completed": 0,
        "fixture_days_failed": 0,
        "fixtures_received": 0,
        "fixtures_created": 0,
        "fixtures_updated": 0,
        "data_selected": 0,
    ...
    return {
        ...
        "max_months_per_request": MAX_MONTHS_PER_RUN,
        "leagues": leagues or [],
