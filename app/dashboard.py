...
from app.forward_test_ledger import DecisionRecord, ensure_forward_test_schema, router as forward_test_router

DASHBOARD_VERSION = "dashboard_v1"
...
def _summary_payload(days: int) -> dict[str, Any]:
    ...
    with SessionLocal() as session:
        total_records = int(
            session.scalar(select(func.count(DecisionRecord.id)).where(*filters)) or 0
        )
        bet_records = int(
            session.scalar(
                select(func.count(DecisionRecord.id)).where(
                    *filters, DecisionRecord.decision == "BET"
                )
            )
            or 0
        )
        no_bet_records = int(
            session.scalar(
                select(func.count(DecisionRecord.id)).where(
                    *filters, DecisionRecord.decision == "NO_BET"
                )
            )
            or 0
        )
        ...
        pending_records = int(
            session.scalar(
                select(func.count(DecisionRecord.id)).where(
                    *filters, DecisionRecord.settlement_status == "UNSETTLED"
                )
            )
            or 0
        )
        ...
        pnl_units = _as_float(
            session.scalar(
                select(func.coalesce(func.sum(DecisionRecord.hypothetical_pnl_units), 0)).where(
                    *filters,
                    DecisionRecord.decision == "BET",
                    DecisionRecord.settlement_status == "SETTLED",
                )
        ...
        avg_odd = _as_float(
            session.scalar(
                select(func.avg(DecisionRecord.selected_odd)).where(
                    *filters, DecisionRecord.decision == "BET"
                )
            )
        )
        avg_edge_pct = _as_float(
            session.scalar(
                select(func.avg(DecisionRecord.edge_percentage_points)).where(
                    *filters, DecisionRecord.decision == "BET"
                )
            )
        )
        avg_ev_pct = _as_float(
            session.scalar(
                select(func.avg(DecisionRecord.expected_value_pct)).where(
                    *filters, DecisionRecord.decision == "BET"
                )
            )
        )
        ...
        items.append(
            {
                ...
                "fixture_starts_at": record.fixture_starts_at.isoformat() if record.fixture_starts_at else None,
                ...
                "selected_odd": _as_float(record.selected_odd) if record.selected_odd is not None else None,
                "edge_pct": _as_float(record.edge_percentage_points) if record.edge_percentage_points is not None else None,
                "expected_value_pct": _as_float(record.expected_value_pct) if record.expected_value_pct is not None else None,
                "calibrated_confidence": _as_float(record.calibrated_favorite_confidence) if record.calibrated_favorite_confidence is not None else None,
                "settlement_status": record.settlement_status,
                "actual_result": record.actual_result,
                "selection_won": selection_won,
                "pnl_units": _as_float(record.hypothetical_pnl_units) if record.hypothetical_pnl_units is not None else None,
            }
        )
...
<head>
  ...
  <style>
    :root{--bg:#081018;--panel:#101b26;--panel2:#132230;--text:#e9f0f6;--muted:#8fa2b5;--line:#223446;--good:#54d39a;--bad:#ff7b7b;--accent:#79a8ff;--warn:#f0c36a}
    *{box-sizing:border-box} body{margin:0;background:linear-gradient(180deg,#071019,#0b141d 48%,#081018);color:var(--text);font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}
    .wrap{max-width:1440px;margin:0 auto;padding:28px}.top{display:flex;align-items:flex-end;justify-content:space-between;gap:18px;margin-bottom:24px}.brand h1{margin:0;font-size:30px;letter-spacing:-.04em}.brand p{margin:7px 0 0;color:var(--muted)}
    .controls{display:flex;gap:10px;align-items:center}.badge{border:1px solid #34506a;background:#0f1e2a;color:#9bc5ff;padding:8px 11px;border-radius:999px;font-size:12px;font-weight:700;letter-spacing:.08em}.select{background:#0f1b26;color:var(--text);border:1px solid var(--line);padding:9px 12px;border-radius:10px}
    .grid{display:grid;grid-template-columns:repeat(6,minmax(0,1fr));gap:12px}.card{background:rgba(16,27,38,.92);border:1px solid var(--line);border-radius:14px;padding:16px;min-height:104px}.label{color:var(--muted);font-size:12px;text-transform:uppercase;letter-spacing:.08em}.value{font-size:28px;font-weight:760;margin-top:10px;letter-spacing:-.04em}.sub{font-size:12px;color:var(--muted);margin-top:6px}
    .section{margin-top:16px;display:grid;grid-template-columns:1.4fr 1fr;gap:16px}.panel{background:rgba(16,27,38,.92);border:1px solid var(--line);border-radius:14px;padding:18px}.panel h2{font-size:15px;margin:0 0 14px}.chart{height:260px;width:100%;display:block}.axis{stroke:#294053;stroke-width:1}.line{fill:none;stroke:#79a8ff;stroke-width:3;vector-effect:non-scaling-stroke}.zero{stroke:#506579;stroke-dasharray:5 5}.empty{display:flex;align-items:center;justify-content:center;height:220px;color:var(--muted)}
    table{width:100%;border-collapse:collapse;font-size:13px}th{color:var(--muted);font-weight:600;text-align:left;padding:10px 8px;border-bottom:1px solid var(--line)}td{padding:11px 8px;border-bottom:1px solid rgba(34,52,70,.65);vertical-align:middle}.right{text-align:right}.good{color:var(--good)}.bad{color:var(--bad)}.muted{color:var(--muted)}.decision{font-weight:700}.pill{display:inline-flex;padding:4px 7px;border-radius:7px;background:#162737;border:1px solid #294158;font-size:11px}
    .records{margin-top:16px}.scroll{overflow:auto}.loading{padding:40px;color:var(--muted);text-align:center}.error{padding:18px;border:1px solid #683b3b;background:#261616;border-radius:12px;color:#ffb4b4;margin-top:16px}
    @media(max-width:1050px){.grid{grid-template-columns:repeat(3,1fr)}.section{grid-template-columns:1fr}}@media(max-width:650px){.wrap{padding:18px}.top{align-items:flex-start;flex-direction:column}.grid{grid-template-columns:repeat(2,1fr)}.card{min-height:94px}.value{font-size:23px}}
  </style>
</head>
<body>
<div class="wrap">
  <div class="top">
    <div class="brand"><h1>Enigma Core</h1><p>Decision Intelligence Dashboard · Forward Test</p></div>
    <div class="controls"><span class="badge">RESEARCH ONLY</span><select id="days" class="select"><option value="7">7 dias</option><option value="30">30 dias</option><option value="90" selected>90 dias</option><option value="365">1 ano</option><option value="0">Todo período</option></select></div>
  </div>
...
function chart(curve){if(!curve?.length)return '<div class="empty">Sem BETs liquidadas no período.</div>';const vals=curve.map(x=>Number(x.cumulative_pnl_units||0));const w=800,h=240,p=24,min=Math.min(0,...vals),max=Math.max(0,...vals),span=(max-min)||1;const pts=vals.map((v,i)=>{const x=p+(i*(w-2*p)/Math.max(1,vals.length-1));const y=p+((max-v)*(h-2*p)/span);return `${x.toFixed(1)},${y.toFixed(1)}`}).join(' ');const zy=p+((max-0)*(h-2*p)/span);return `<svg class="chart" viewBox="0 0 ${w} ${h}" preserveAspectRatio="none"><line class="zero" x1="${p}" x2="${w-p}" y1="${zy}" y2="${zy}"/><polyline class="line" points="${pts}"/></svg>`}
...
<div class="card"><div class="label">BETs liquidadas</div><div class="value">${k.settled_bets}</div><div class="sub">${k.pending_records} registros pendentes</div></div>
...
<div class="card"><div class="label">P&L hipotético</div><div class="value ${cls(k.pnl_units)}">${fmt(k.pnl_units)}u</div><div class="sub">1 unidade por BET</div></div>
<div class="card"><div class="label">ROI</div><div class="value ${cls(k.roi_pct)}">${fmt(k.roi_pct,1)}%</div><div class="sub">sobre BETs liquidadas</div></div>
</div>
