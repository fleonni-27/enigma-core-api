...
BASELINE_POLICY_VERSION = "baseline_1x2_confidence_policy_v2"
CLASS_ORDER = ["1", "X", "2"]
...
def _policy_evaluation(rows: list[dict], y_true: np.ndarray, probabilities: np.ndarray, include_predictions: bool) -> dict:
    ...
    tier_stats = {name: {"rows": 0, "hits": 0, "single_rows": 0, "double_chance_rows": 0, "rec_prob_sum": 0.0, "fav_prob_sum": 0.0, "fav_hits": 0} for name in tier_names}

    total_hits = single_rows = single_hits = dc_rows = dc_hits = 0
    ...
    for row, actual, probs in zip(rows, y_true, probabilities):
        ...
        actual = str(actual)
        hit = actual in decision["accepted_outcomes"]
        favorite_hit = actual == decision["favorite"]
        total_hits += int(hit)
        ...
        stats["rows"] += 1
        stats["hits"] += int(hit)
        stats["fav_hits"] += int(favorite_hit)
        stats["rec_prob_sum"] += decision["recommendation_probability"]
        stats["fav_prob_sum"] += decision["favorite_probability"]
        ...
        if include_predictions:
            prediction_rows.append({
                "fixture_id": row.get("fixture_id"), "sportmonks_fixture_id": row.get("sportmonks_fixture_id"),
                "starts_at": row.get("starts_at"), "league": row.get("league"), "home_team": row.get("home_team"), "away_team": row.get("away_team"),
                "actual": actual, "tier": decision["tier"], "favorite": decision["favorite"],
                "favorite_probability": round(decision["favorite_probability"], 6), "decision_type": decision["decision_type"],
                "recommendation": decision["recommendation"], "recommendation_probability": round(decision["recommendation_probability"], 6),
                "hit": bool(hit), "probabilities": {k: round(v, 6) for k, v in decision["probabilities"].items()},
            })
    ...
    for name in tier_names:
        s = tier_stats[name]
        n = s["rows"]
        avg_fav = s["fav_prob_sum"] / n if n else None
        realized_fav = s["fav_hits"] / n if n else None
        tiers[name] = {
            "rows": n,
            "share_pct": round(n / len(y_true) * 100, 2) if len(y_true) else 0.0,
            "hits": s["hits"],
            "hit_rate": round(s["hits"] / n, 6) if n else None,
            "single_rows": s["single_rows"],
            "double_chance_rows": s["double_chance_rows"],
            "avg_recommendation_probability": round(s["rec_prob_sum"] / n, 6) if n else None,
            "calibration": {
                "avg_favorite_probability": round(avg_fav, 6) if avg_fav is not None else None,
                "realized_favorite_win_rate": round(realized_fav, 6) if realized_fav is not None else None,
                "calibration_gap": round(realized_fav - avg_fav, 6) if avg_fav is not None else None,
            },
        }
    ...
    result = {
        ...
        "single": {"rows": single_rows, "share_pct": round(single_rows / len(y_true) * 100, 2), "hits": single_hits, "hit_rate": round(single_hits / single_rows, 6) if single_rows else None},
        "double_chance": {"rows": dc_rows, "share_pct": round(dc_rows / len(y_true) * 100, 2), "hits": dc_hits, "hit_rate": round(dc_hits / dc_rows, 6) if dc_rows else None},
        "tiers": tiers,
    }
    if include_predictions:
...
def build_baseline_1x2_confidence_policy_v2(
    start_date: date,
    end_date: date,
    ...
    model_dataset = build_model_dataset_v1(start_date=start_date, end_date=end_date, leagues=leagues, lookback_matches=lookback_matches, min_history_matches=min_history_matches, train_ratio=train_ratio, validation_ratio=validation_ratio, max_rows=max_rows, include_rows=True)
    ...
    metadata = {"version": BASELINE_POLICY_VERSION, "model_dataset_sha256": model_dataset["model_dataset_sha256"], "family_sha256": family_payload["family_sha256"], "family": family, "class_weight_balanced": class_weight_balanced, "thresholds": {"strong": 0.70, "effective": 0.60, "moderate": 0.50}, "moderate_action": "favorite_plus_draw", "open_action": "favorite_plus_draw"}
    policy_sha = _stable_hash(metadata)

    return {
        "status": "ok", "version": BASELINE_POLICY_VERSION, "policy_id": f"{BASELINE_POLICY_VERSION}:{policy_sha[:16]}", "policy_sha256": policy_sha, "family": family,
        "parent": {"model_dataset_id": model_dataset["model_dataset_id"], "model_dataset_sha256": model_dataset["model_dataset_sha256"], "family_sha256": family_payload["family_sha256"], "split_sha256": model_dataset["parent"]["split_sha256"], "dataset_sha256": model_dataset["parent"]["dataset_sha256"]},
        "training": {"algorithm": "multinomial_logistic_regression", "feature_count": len(feature_names), "train_rows": len(train_rows), "validation_rows": len(validation_rows), "test_rows": len(test_rows), "class_weight": "balanced" if class_weight_balanced else None, "fit_scope": "train partition only"},
        "decision_policy": {
            "STRONG_FAVORITE": {"favorite_probability": ">=0.70", "action": "single favorite"},
            "EFFECTIVE_FAVORITE": {"favorite_probability": "0.60-0.699999", "action": "single favorite"},
            "MODERATE_FAVORITE": {"favorite_probability": "0.50-0.599999", "action": "double chance favorite+draw (1X/X2)"},
            "OPEN_GAME": {"favorite_probability": "<0.50", "action": "double chance favorite+draw (1X/X2)"},
        ...
        "policy": {"temporal_split_preserved": True, "shuffle": False, "train_only_fit": True, "validation_used_for_fit": False, "test_used_for_fit": False, "thresholds_are_fixed_before_evaluation": True, "calibration_report_is_diagnostic_only": True, "double_chance_is_evaluation_of_accepted_outcomes_not_probability_recalibration": True, "target_match_postgame_data_as_features": False, "deterministic": True},
    }
