...
async def backfill_fixtures(start_date: date, end_date: date) -> dict:
    ...
    if day_count > MAX_BACKFILL_DAYS:
        return {
            ...
            "detail": f"Maximum range per request is {MAX_BACKFILL_DAYS} days",
    ...
    while current <= end_date:
        try:
            ...
            day_result = {
                ...
                "received": result.get("received", len(payload.get("data") or [])),
    ...
    overall_status = "ok" if totals["failed_days"] == 0 else "partial"
    return {
        "status": overall_status,
        "start_date": start_date.isoformat(),
        "end_date": end_date.isoformat(),
        "max_days_per_request": MAX_BACKFILL_DAYS,
        "totals": totals,
        "days": per_day,
    }
