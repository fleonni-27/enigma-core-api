...
def build_data_coverage_report(start_date: date, end_date: date) -> dict:
    ...
    with SessionLocal() as session:
        fixtures = session.scalars(
            ...
            .order_by(Fixture.starts_at.asc())
        ).all()
        ...
        if fixture_ids:
            snapshots = session.scalars(
                ...
                .order_by(FixtureDataSnapshot.fixture_id.asc(), FixtureDataSnapshot.fetched_at.desc())
            ).all()
    ...
    for fixture in fixtures:
        ...
        if snapshot is None:
            continue
        bucket["snapshots"] += 1
        if _has_payload(snapshot.lineups):
            bucket["with_lineups"] += 1
        if _has_payload(snapshot.statistics):
            bucket["with_statistics"] += 1
        if _has_payload(snapshot.xg):
            bucket["with_xg"] += 1
    ...
    for key, bucket in buckets.items():
        ...
        report.append(
            {
                "key": None if key.startswith("unmapped:") else key,
                ...
                "lineups_fixtures": bucket["with_lineups"],
                "statistics_fixtures": bucket["with_statistics"],
                "xg_fixtures": bucket["with_xg"],
                "lineups_pct": round(bucket["with_lineups"] / fixtures_count * 100, 1) if fixtures_count else 0.0,
                "statistics_pct": round(bucket["with_statistics"] / fixtures_count * 100, 1) if fixtures_count else 0.0,
                "xg_pct": round(bucket["with_xg"] / fixtures_count * 100, 1) if fixtures_count else 0.0,
            }
        )
